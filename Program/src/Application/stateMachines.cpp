...
static bool isUserRegistered(const MFRC522::UID_st& uid) {
    ...
    for (size_t index = 0; index < userCount; index++) {
        ...
        for (size_t j_index = 0; j_index < RFID_USER_UID_SIZE; j_index++) {
            result = g_eeprom->read(&registeredUID[j_index], M24C16::UINT8,
                                    index * RFID_USER_UID_SIZE + USERS_INIT_POSITION + j_index);
            if (result != EEPROM_OK) {
                return false;
            }
        }
...
void userRegistrationStateMachine(UserRegistrationState& state) {
    switch (state) {
        ...
        case UserRegistrationState::WAITING_FOR_USER:
            ...
            while (result == RFID_BUSY) {
                result = g_rfid->getUID(&uuid); //TODO: This functions is not correctly implemented
            }

            if (result == RFID_SUCCESS) {
                if (!isUserRegistered(uuid)) {
                    uint8_t registerAttempts = 0;
                    while (!registerNewUser(uuid) && registerAttempts < 3) {
                        registerAttempts++;
                    }
                }
