...
void PWM::initPWM(void) const {
	...
	SCT->CONFIG |= (1 << 0) | (1 << 17);
	SCT->MATCH[0] = FREQ_CLOCK_MCU / g_systick_freq;
	SCT->MATCHREL[0] = FREQ_CLOCK_MCU / g_systick_freq;
...
void PWM::setDuty(float duty) {
    ...
    if (this->m_activity == Gpio::HIGH) duty /= duty;
...
void PWM::enable(void) {
    ...
    SCT->OUT[this->m_channel].CLR = (1 << 1); // Turn off the following channel
